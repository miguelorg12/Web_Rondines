...
function Sucursales() {
  ...
  const handleSaveBranch = async () => {
    if (!companyId || !userId) {
      alert("Debes seleccionar una empresa y un usuario");
      return;
    }
  ...
  return (
    <Paper sx={{ width: "100%", overflow: "hidden", p: 2 }}>
            ...
            {filteredData
              ...
              .map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>{getCompanyName(row.company_id)}</TableCell>
      ...
      <GenericModal
        ...
        disabled={!name.trim() || !address.trim() || !companyId || !userId || companies.length === 0 || users.length === 0}
      >
        <Box
          sx={{
            display: "flex",
