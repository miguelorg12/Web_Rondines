...
import { useGetUsersBySpecificRolesQuery } from "../redux/users/usersApi";
...
function Sucursales() {
  ...
  const { data: users = [], isLoading: isLoadingUsers } = useGetUsersBySpecificRolesQuery();
  ...
  // Debug para validación del botón
  useEffect(() => {
    if (openModal) {
      const isDisabled = !name.trim() || !address.trim() || (editingBranch ? false : (companyId === 0 || userId === 0)) || companies.length === 0 || users.length === 0 || isLoadingCompanies || isLoadingUsers;
      console.log("🔍 Validación del botón Sucursales:", {
        name: name.trim(),
  ...
  }, [openModal, name, address, companyId, userId, editingBranch, companies.length, users.length, isLoadingCompanies, isLoadingUsers]);
  
  // Asegurar que branches sea un array
  const safeBranches = Array.isArray(branches) ? branches : [];
  
  const [createBranch, { isLoading: isCreating, error: createError, reset: resetCreateError }] = useCreateBranchMutation();
  const [updateBranch, { isLoading: isUpdating, error: updateError, reset: resetUpdateError }] = useUpdateBranchMutation();
  const [deleteBranch, { isLoading: isDeleting, error: deleteError, reset: resetDeleteError }] = useDeleteBranchMutation();
  ...
  const handleOpenModal = (branch?: Branch) => {
    if (branch) {
      ...
      // Usar los IDs de los objetos anidados si existen, o 0 si son null
      setCompanyId(branch.company?.id || 0);
      setUserId(branch.user?.id || 0);
  ...
  const handleSaveBranch = async () => {
    // Solo validar companyId y userId en modo creación
    if (!editingBranch && (companyId === 0 || userId === 0)) {
      showToast("Debes seleccionar una empresa y un usuario", "error");
      return;
    }
    ...
    if (editingBranch) {
      ...
      try {
        ...
        showToast(`Error al actualizar sucursal: ${apiError.status || 'Error desconocido'}`, "error");
      }
    } else {
      ...
      console.log('🔍 Datos de creación:', newBranch);
      console.log('🔍 companyId tipo:', typeof companyId, 'valor:', companyId);
      console.log('🔍 userId tipo:', typeof userId, 'valor:', userId);
      ...
      try {
        ...
        showToast("Error al crear sucursal", "error");
      }
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    if (branchToDelete) {
      try {
        ...
        showToast("Error al eliminar sucursal", "error");
      }
    }
  };
  ...
  return (
    <Paper sx={{ width: "100%", overflow: "hidden", p: 2 }}>
      ...
      <GenericModal
        ...
        disabled={!name.trim() || !address.trim() || (editingBranch ? false : (companyId === 0 || userId === 0)) || companies.length === 0 || users.length === 0 || isLoadingCompanies || isLoadingUsers}
        onErrorClear={() => {
          if (createError) resetCreateError();
      ...
      >
        ...
        >
          ...
          <FormControl fullWidth sx={{ flex: 1 }}>
            ...
            >
              <MenuItem value={0}>Seleccionar manager</MenuItem>
              {users.map((user) => (
