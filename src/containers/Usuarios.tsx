...
function Usuarios() {
  ...
  useEffect(() => {
    if (openModal) {
      const isDisabled = isCreating || isUpdating || !name.trim() || !lastName.trim() || !curp.trim() || !email.trim() || (!isEditMode && !password.trim()) || (!isEditMode && (!roleId || !branchId));
    ...
    if (password !== confirmPassword) {
      ...
      return;
       ...
       if (isEditMode && editingUser) {
         ...
         // Estos campos solo se usan para crear usuarios, no para actualizar
         

         
         console.log("🔍 Datos que se van a enviar en update:", updateData);
         await updateUser({ 
           id: editingUser.id, 
           data: updateData
         }).unwrap();
      ...
      refetch(); // Refrescar la lista
       ...
       console.error("userData enviado:", isEditMode ? "updateData" : "createData");
        ...
        <DialogContent>
          <Box sx={{ pt: 1 }}>
            ...
            >
                 ...
                 className="custom-textfield"
                 required={!isEditMode}
                 placeholder={isEditMode ? "Dejar vacío para mantener la contraseña actual" : "Mínimo 12 caracteres, mayúscula, minúscula, número y símbolo"}
               />
               <ValidatedTextField
                 ...
                 className="custom-textfield"
                 required={!isEditMode}
                 placeholder={isEditMode ? "Dejar vacío para mantener la contraseña actual" : "Repite la contraseña"}
               />
            </Box>
                                       <Box
                sx={{
