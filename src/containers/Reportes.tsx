...
function Reportes() {
  ...
  const filteredData = reportesData.filter(
    (item) =>
      (item.numReporte.toLowerCase().includes(filterText.toLowerCase()) ||
        item.guardia.toLowerCase().includes(filterText.toLowerCase()) ||
        item.sucursal.toLowerCase().includes(filterText.toLowerCase()) ||
        item.incidencia.toLowerCase().includes(filterText.toLowerCase()) ||
    ...
    <Paper sx={{ width: "100%", overflow: "hidden", p: 2 }}>
      ...
      <TableContainer className="custom-table-container">
        <Table className="custom-table" stickyHeader>
          <TableHead>
            <TableRow>
              <TableCell>Num reporte</TableCell>
          ...
          <TableBody>
            {filteredData
              ...
              .map((row) => (
                <TableRow key={row.id}>
                  <TableCell>{row.numReporte}</TableCell>
                  <TableCell>{row.guardia}</TableCell>
                  <TableCell>{row.sucursal}</TableCell>
                  <TableCell>{row.incidencia}</TableCell>
